...
public class MysticFactory {
	...
	public static ItemStack createItem(MysticType type, int chestTier) {
		...
		if(type == MysticType.SWORD) fresh = FreshCommand.getFreshItem("sword");
			...
			if(rand <= 0.2) fresh = FreshCommand.getFreshItem("red");
			else if(rand <= 0.4) fresh = FreshCommand.getFreshItem("orange");
			else if(rand <= 0.6) fresh = FreshCommand.getFreshItem("yellow");
